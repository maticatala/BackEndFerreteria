...
export class OrdersController {
  ...
  @Post()
  create(@Body() createOrderDto: CreateOrderDto, @CurrentUser() currentUser: UserEntity): Promise<OrderEntity> {
  ...
  @UseGuards(AuthenticationGuard)
  @Get('/userOrder/:id')
  getUserOrderById(@Param('id') id: string, @CurrentUser() currentUser: UserEntity): Promise<OrderEntity> {
    return this.ordersService.getUserOrderById(+id, currentUser);
  }

  @UseGuards(AuthenticationGuard, AuthorizeGuard([Roles.ADMIN]))
  @Put(':id')
  update(@Param('id') id: string, @Body() updateOrderStatusDto: UpdateOrderStatusDto, @CurrentUser() currentUser: UserEntity) {
    return this.ordersService.update(+id, updateOrderStatusDto, currentUser);
  }

  @UseGuards(AuthenticationGuard, AuthorizeGuard([Roles.ADMIN]))
  @Put('payments/:id')
  updatePayment(@Param('id') id: string, @Body() updatePaymentStatusDto: UpdatePaymentStatusDto, @CurrentUser() currentUser: UserEntity) {
